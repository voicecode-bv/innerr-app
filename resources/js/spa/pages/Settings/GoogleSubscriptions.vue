...
const tiers = computed<Tier[]>(() => [
    {
        ...
        icon: diamondIcon,
        monthly: findProduct('plus_google_monthly'),
        yearly: findProduct('plus_google_yearly'),
    },
    {
        key: 'pro',
        title: t('Familie+'),
        ...
        icon: crownIcon,
        monthly: findProduct('pro_google_monthly'),
        yearly: findProduct('pro_google_yearly'),
    },
]);
...
async function loadProducts(): Promise<void> {
    try {
        ...
        console.log('[GoogleSubscriptions] products result', result);
        ...
        console.error('[GoogleSubscriptions] products error', e);
...
async function purchase(productId: string): Promise<void> {
    ...
    try {
        ...
        console.log('[GoogleSubscriptions] purchase result', result);
        ...
        console.error('[GoogleSubscriptions] purchase error', e);
        purchaseDebug.value = {
            label: t('Purchase debug — :id', { id: productId }),
...
async function restore(): Promise<void> {
    ...
    try {
        ...
        console.log('[GoogleSubscriptions] restore result', result);
        ...
        console.error('[GoogleSubscriptions] restore error', e);
        purchaseDebug.value = {
            label: t('Restore debug'),
            outcome: e.code === 'user_cancelled' ? 'cancelled' : 'error',
            payload: JSON.stringify(
            ...
            <div class="relative space-y-4 px-4 pt-4 pb-24">
                ...
                <template v-else>
                    ...
                    <p class="mt-2 text-xs leading-relaxed text-sand-500 dark:text-sand-400">
                        {{ t('Subscriptions automatically renew at the end of each billing period unless cancelled at least 24 hours before. Your Google account will be charged for renewal at the same price. You can manage and cancel your subscription in the Google Play account settings.') }}
                    </p>
